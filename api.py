...
def get_books_url(url_category):  # retourne une liste avec l' ensemble des url des articles pour une
    # catégorie donnée
    links = []
    ...
    if soup.find("li", class_="current") is None:
        ...
        nbr_of_pages = soup.find("li", class_="current").text[40:41]
        print(nbr_of_pages)
...
x = 0
fin = 1000
for url_cat in create_categories_url():
    with open(f"data/{get_name_category()[x]}.csv", 'a', encoding="utf-8") as f:
        if x == int(stop):
            break
        ...
        create_columns()
        y = 1
        for book in get_books_url(url_cat):
            print(f"livre {y}")
            print(f"il reste encore {fin} livres ....")
            y += 1
            fin -=1
            for item in get_info_1_book(book):
                f.write(str(item))
                f.write(",")
            f.write("\n")